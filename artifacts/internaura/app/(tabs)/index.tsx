...
function FilterSheet({
  ...
  const active =
    ...
    Number(filters.skills.length > 0);
...
export default function Home() {
  const i = useSafeAreaInsets(),
    r = useRouter(),
    { profile, savedIds, internships, recommendations, skipInternship, skippedIds } = useApp();
  ...
  const displayList = recommendations && recommendations.length > 0 ? recommendations : internships;
  ...
  const filtered = useMemo(() => {
    const result = displayList.filter((x: any) => {
      if (skippedIds.includes(x.id)) return false;
      if (filters.domains.length > 0 && !filters.domains.includes(x.domain)) return false;
      if (!matchesWorkMode(x.workMode || x.mode, filters.mode)) return false;
      if (filters.location && filters.location.trim().length > 0) {
        const locFilter = filters.location.trim().toLowerCase();
        const itemLoc = (x.location || x.internship?.location || "").toLowerCase();
        if (!itemLoc.includes(locFilter)) return false;
      }
      if (!matchesStipendFloor(x.stipend, filters.stipend)) return false;
      if (filters.minMatch > 0 && (x.atsScore || x.matchScore || x.match || 0) < filters.minMatch) return false;
      if (!matchesDurationBucket(x.duration, filters.duration as DurationBucket)) return false;
      if (filters.skills.length > 0) {
        const itemSkills = [...(x.requiredSkills || []), ...(x.preferredSkills || [])];
        if (!matchesSkills(itemSkills, filters.skills, filters.skillMode)) return false;
      }
      return true;
    });

    return [...result].sort((a: any, b: any) => compareBySort(a, b, sort as SortKey));
  }, [filters, sort, displayList, skippedIds]);
  ...
  const handleSkipConfirm = (reason: string) => {
    if (skipTarget) {
      skipInternship(skipTarget.id, reason);
      setSkipTarget(null);
    }
  };
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            ...
            <Text style={s.greeting}>Good morning, {profile?.name ? profile.name.split(" ")[0] : "Student"}.</Text>
            ...
            {/* Resume AI Analyzer Entry Card */}
            <View style={s.resumeBannerCard}>
              <View style={s.resumeBannerLeft}>
                <View style={s.resumeBannerIconCircle}>
                  <Feather name={profile?.skills && profile.skills.length >= 3 ? "file-text" : "zap"} size={20} color={c.primary} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={s.resumeBannerTitle}>
                    {profile?.skills && profile.skills.length >= 3 ? "Update Resume AI Analysis" : "Boost Match Accuracy with Resume AI"}
                  </Text>
                  <Text style={s.resumeBannerSub}>
                    {profile?.skills && profile.skills.length >= 3
                      ? "Re-parse your latest PDF resume to update skill rankings & Adzuna job matches."
                      : "Upload your PDF resume to auto-extract verified skills & unlock targeted internship matches."}
                  </Text>
                </View>
              </View>
              <Pressable onPress={() => r.push("/resume-analyzer")} style={s.resumeBannerBtn}>
                <Text style={s.resumeBannerBtnText}>
                  {profile?.skills && profile.skills.length >= 3 ? "Update Resume" : "Upload Resume"}
                </Text>
            ...
            <Text style={s.count}>
              Showing {filtered.length} of {displayList.length} internships
            </Text>
